...
import { fetchCategoryClothes } from '../../../../API/api/Clothes/clothes_api';
...
export default function Shoes_Brand(props){
    ...
    const FetchShoesList = async (value) =>{
        const _BrandShoes = await fetchCategoryClothes(value);
    ...
    async function FirstFetchList(){
        const _BrandShoes = await fetchCategoryClothes(brandtags[0].query);
    ...
    useEffect(() =>{
        ...
        if(brandtags.length === 0){
            ...
            FetchShoesList(brandtags[brand].query)
        }
    },[brand])
                                            ...
                                            shoes.map((item,index)=>{
                                                return(
                                                    <SwiperSlide className='shoes'>
                                                        <Box onClick={()=>navigateToClothesDetail(item.id)} sx={{width:'100%',height:'250px'}}>
